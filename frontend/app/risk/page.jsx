...
function RiskInner() {
  ...
  const isShort = entry > 0 && stop > entry;
  const perShare = entry && stop ? Math.abs(entry - stop) : 0;
  const valid = entry > 0 && stop > 0 && perShare > 0;
  const qty = valid ? Math.floor(riskAmt / perShare) : 0;
  const posValue = qty * entry;
  const gain = isShort ? entry - target : target - entry;
  const rr = valid && target > 0 && gain > 0 ? gain / perShare : null;
    ...
    <div className="space-y-4 max-w-3xl">
      ...
      {valid ? (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          {[[isShort ? "Shares to short" : "Shares to buy", qty.toLocaleString("en-IN"), "text-brass"],
            ...
            ["Gain at target", rr ? "+" + fmt(qty * gain, 0) : "—", "text-up"],
          ].map(([k, v, cls]) => (
        ...
        <div className="card text-mut text-sm">Enter an entry and a stop — below entry for a long, above for a short — or load a symbol's ATR plan to see position size.</div>
      )}
